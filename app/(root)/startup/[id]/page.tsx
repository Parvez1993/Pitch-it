          ...
          <div className="max-w-4xl mx-auto">
            ...
            <ul className="mt-7 card_grid-sm">
              {editorPosts.map((post: StartupTypeCard, i: number) => (
                <BusinessPlanCard key={i} item={post} />
              ))}
            </ul>
          </div>
        )}

        {/* //Some of the topics that the viewer may find useful*/}
        {/* //using ppr here the above what you see in static */}
      </section>
      <Suspense fallback={<Skeleton className="view_skeleton" />}>
        <View id={id} />
      </Suspense>
    </>
  ); // Assuming `post` has a `title` field
};
